...
- **多种导出格式:** 支持将数据导出为 **Excel, PDF, 和 CSV** 格式，以适应不同的分析和归档需求。
- **灵活的照片选项:**
  1. **无照片导出**：仅导出文字信息。
  2. **包含首张照片**：每类照片（收集前/后）导出第一张。
  3. **包含全部照片**：导出记录中所有关联的照片。
- **自定义报表:** (管理员功能) 提供一个报表生成器，允许用户自定义报表内容，例如选择特定的时间范围、数据列、并按单位或废物类型进行分组统计。
- **定时报表:** (管理员功能) 支持设置定时任务，系统可以自动在每周或每月生成关键数据报表，并发送到指定人员的邮箱。
...
### 3.5 可定制的仪表盘 (Dashboard)

为了提供更个性化和高效的数据概览，系统引入了可定制的仪表盘功能。每个用户都可以根据自己的角色和关注点，自由选择和排列仪表盘上显示的数据模块。

- **模块化设计:** 仪表盘由多个独立的数据组件（或称为“小部件”）构成，例如：
  - 近期废物总量趋势图
  - 各单位填报率统计
  - 个人待办事项
  - 系统公告
- **个性化配置:** 用户可以通过简单的拖拽操作，自定义仪表盘的布局，并选择显示或隐藏特定的小部件。
- **角色预设:** 系统为不同角色（如超级管理员、单位管理员、基层员工）提供了默认的仪表盘布局，确保用户首次登录即可看到最相关的信息。

**优点:**
- **信息获取更直观:** 用户可以快速获取对自己最重要的信息，提高决策效率。
- **提升用户满意度:** 个性化的功能设计能更好地满足不同用户的需求。
...
- **触屏优化**：支持手势操作
- **移动拍照**：直接调用手机相机拍照上传
- **增强的离线功能**：不仅支持离线查看已加载的数据，还允许用户在无网络连接的情况下**创建和编辑**废物记录。这些更改会缓存在本地，并在网络恢复后自动同步至服务器，极大地便利了现场作业。

### 5.4 地图可视化

- **全局数据概览**：在地图上直观展示所有废物的产生位置和分布情况。
- **智能点聚合**：当数据点过多时，自动进行聚合显示，避免地图拥挤，点击聚合点可散开查看详情。
- **详细信息交互**：点击地图上的标记点，可弹出信息窗体，显示废物的详细信息（单位、类型、重量、照片等）。
...
**Q：忘记密码怎么办？**
A：请在登录页面点击“忘记密码”链接，通过注册时使用的邮箱或手机号进行身份验证，即可自行重置密码。如果此方法无效，再联系管理员进行处理。
...
- **2025-06-03**：新增地图可视化功能，支持数据点聚合展示
